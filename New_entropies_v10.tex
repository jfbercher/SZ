...
% Bol96, 98
\SZ{Since the  pionner works  Boltzmann, Maxwell, Planck  and Gibbs~\cite{Bol64,
    Gib02,  Pla15,  Nie52:v2,  Jay65,  MulMul09},  von  Neuman~\cite{vNeu27}  or
  Shannon~\cite{Sha48} on the  entropy as a tool for  uncertainty or information
  measure, many  investigations were devoted to generalization  of the so-called
  Shannon  entropy  and its  associated  measures~\cite{Ren61, Var66,  HavCha67,
    Csi67, Dar70,  AczDar75, DarJar79, Tsa88, Sal87,  SalMen93, Sal94, LieVaj06,
    Bas13}.   If the  Shannon  measures  are very  powerfull  especially in  the
  communication domain, for  compression purposes, many generalizations proposed
  latter  on   has  also  showed  promising   interpretations  and  applications
  (Panter-Dite formula  in quantification where the  R\'enyi or Havdra-Charv\'at
  entropy emerges~\cite{PanDit51, Llo82, GerGra92}, codification penalizing long
  ...
  $(h,\phi)$-entropies~\cite{Csi67, BurRao82,  SalMen93, Sal94, MenMor97, Par06,
    BosZoz16}.   Such  a  general  class  (or more  precisely  the  subclass  of
  $\phi$-entropies) traced  back to the  work of Burbea  \& Rao~\cite{BurRao82}.
  Since, many works took hold of  these entropies. Not only they offer a general
  framework to study general properties shared by various special entropies, but
  also  they offer  various  potential applications  as  described for  instance
  in~\cite{Par06,  CreRea84,   MenMor97:5,  Par99,  Par99:1,   CrePar00,  Arn01,
    ZozPor08, ZozBos13, ZozBos14, BosZoz16}.  Note that if a huge amount of work
  deals  with  divergences, entropies  are  special  cases  where the  reference
  measure is taken uniform.
...
The paper is organized  as follows. \SZ{In section~\ref{sec:MaxPhiEnt} we recall
  ...
  of~\cite{KesKap89, KesKap90},  we present  a sufficient condition  linking the
  entropic functional  and the maximizing  distribution, allowing to  both solve
  the direct and the inverse problems.}
% In  passing, we show  that the Bregman  divergence between the MaxEnt  and any
% distribution  sharing the  same moments (the  MaxEnt being the  reference one)
% reduces  to a diference  of entropies, which  potential motivate their  use in
%  goodness-of-fit tests.  In  this same  section,  we then  reverse the  MaxEnt
% problem: fixing the probability distribution and a set of moments, we show the
%   conditions  that   they  must   satisfy  to   be  able   to   determine  the
% distribution-adapted  entropic functional and we formally  determine this last
% one.
...
maximal  entropy distribution.   \SZ{Finally, in  section~\ref{sec:deBruijn}, we
  show  that the  extended quantities  allows to  obtain an  extended  de Bruijn
  identity, profided  the distribution follows a non-linear  heat equation. Some
  exemple  of  determination of  $\phi$-entropies  solving  the inverse  maximum
  entropy problem are  provided in a short series of  appendix, showing in other
  that}  the usual quantities  are recovered  in the  well known  cases (Gausian
distribution, Shannon entropy, Fisher information, variance).
...
\SZ{Let us  here recall the maximum  entropy problem that consists  in searching for
...
but   this   approach   requires   mild   conditions~\cite{KesKap89,   KesKap90,
  BorLew91:03, BorLew93, BenBor92, TebVaj93, Gir97}.  \SZ{A sufficient condition
  condition relating  $f$ and $\phi$ so that  $f$ is the (resp.   $\phi$) is the
  desired solution of the problem is then obtained, as recalled in the following
...
\SZ{As stated in the introduction, two inverse problems can be considered started
  from a given distribution $f$.  These problems were also considered by Kesavan
  \&  Kapur in~\cite{KesKap89,  KesKap90}, although  formalized in  the discrete
  (and essentially Shannon) framework.
...
%
% Do you speak science, p. 89
\begin{enumerate}[label=(C\arabic*)]
\item\label{MonotonyDomains:cond}   $f(x)$   and   $\displaystyle   \sum_{i=1}^n
  \lambda_i  \,  T_i(x)$  must  have  \SZ{the  same  directions  of  variation},
...
\SZ{Note  that  the  generalized  entropy  of  free  energy  functional  defined
  in~\cite{New84}   (see   also~\cite{BlaBon09})   falls   in  the   family   of
  state-dependent  $\phi$-entropy where $\phi(x,y)  = \frac{y^m}{m-1}  + \frac12
  \|x\|^2 y$.}
...
\SZ{Clearly, for  a scalar parameter $\theta$,}  for $K = \frac12$  and $\beta =
2$, the standard heat-equation is recovered.  The case where $\phi(y) = y^q$ was
intensively  studied  in~\cite{Ber13:08}.  \SZ{In  such  a  case, the  nonlinear
  equation  is known as  the $p$-Laplacian  equation and  appears in  physics as
  description  of  flows in  (generalized)  porous  medium~\cite{Vaz06}. In  the
  general case, this kind of equation  can also be viewed as a generalization of
  the  porous  medium  equation, of  the  fast  diffusion  equation, or  of  the
  nonlinear  Fokker-Planck  equation that  appears  in  flow  in porous  medium,
  nonlinear    heat   transfer,    population   dynamics,    Plasma   radiation,
  etc~\cite{New84, Vaz06, Vaz07, Fra05}
%  (see in  particular~\cite[Ch.~6]{Fra05} where
%  the $\phi$-entropy also appears).
}
...
\SZ{As pointed  out before, in the  scalar context $m=1$ for  the parameter, for
  $K=\frac12$ and $\beta = 2$, the  nonlinear heat equation reduces to the usual
  heat equation. Dealing with the Shannon  entropy, we thus recover the usual de
  Bruijn  identity,  while for  a  more general  function  $\phi$,  some of  the
  extensions  proposed in~\cite{Ber13:08,  JohVig07, Guo09,  TorZoz16_arxiv} are
  recovered (see also~\cite[eqs.~(11)~\&~(20)]{BlaBon09}).}
...
\SZ{In this work, we proposed the extension of the informartion theoretic tools,
  inequalities and identities in  the general framework of the $\phi$-entropies.
  In information theory, the maximum  entropy distribution plays a special role,
  so that the problem is extended here in the general class of $\phi$-entropies,
  and an extension to what  we called {\em state-dependent} $\phi$-entropies and
  {\em  multivariate}  $\phi$-entropies.   In  these contexts  both  the  direct
  problem  (finding  the  maximal  distribution  given the  entropy  and  moment
  constraints) and  the inverse problem (finding the  entropic functionnal given
  the distribution  and moment consraints) are investigated.   To illustrate our
  results, examples  of such  construction of entropic  functional adapted  to a
  distribution are  given in the  appendix.  Various situations are  given, from
  the Gaussian  distribution and second  order constraints, to more  exotic ones
  where state-dependent entropies allow to  solve the inverse problem (both with
  global or  partial moments). As  obviously expected, in the  Gaussian context,
  the  family  of  entropies  contains  the  Shannon  entropy.  In  the  Student
  distribution   context    with   second   order    moment   constraints,   the
  Havrda-Charv\'at (or  Tsallis) entropy belongs  to the family of  entropies we
  obtain. In  such a  case, when the  parameter of  the distribution tends  to a
  certain  limit,  the   distribution  tends  to  a  Gaussian,   and  the  limit
  $\phi$-entropies  family   tends  to  that   obtained  for  the   Gaussian  as
  expected.  Furthemore, as  shown  in this  same  appendix, it  is possible  to
  compute  the  maximal  entropy  without  computing  explicitly  the  entropic
  functional.

  In addition to  te study of the maximum entropy  problem, we proposed extended
  moments   and   Fisher    information,   names   $(\alpha,\phi)$-moments   and
  $(\beta,\phi)$-Fisher informations, measures  associated to the $\phi$-entropy
  with  the  same entropic  functional  $\phi$. This  measures  makes  use of  a
  $\phi$-escort  distribution,  and  contains   the  usual  moments  and  Fisher
  information   as    special   cases,    and   tseveral   extension    of   the
  literature.  Moreover,  we  show   as  all  these  informations  measures  are
  inter-related,   thus   extending    several   usual   information   theoretic
  inequalities, the previous ones being special cases.
  ...
  Havdra-Charv\'at  (or R\'eny,  or Tsallis)  setup  in~\cite{LutYan05, LutLv12,
    Ber13,  Ber13:08,  Ber12:06_1},  based  on Sobolev  or  Gagliardo-Nierenberg
  inequalities~\cite{Agu08}.   Unfortunately, the  sketch  of the  proof of  the
  extended  Stam  inequalities  does  not   apply  in  the  general  context  of
  $\phi$-entropies  and $(\beta,\phi)$-Fisher  information.   Such an  extention
  remain  as a  perspective.  The  entropy power  inequality  was also  slightly
  extended in  the R\'enyi entropy context,  but the introduction  of a deformed
  convolution  operator~\cite{JohVig07}.  Again,   in  the  very  general  setup
  considered here,  finding and extended  entropy power inequality seems  a very
  difficul task and is still under  investigation. Note that one of the proof of
  the  Stam inequality  combines both  the entrop  power inequality  and  the de
  Bruijn identity.  A waypoint to extend  both the entropy  power inequality and
  the Stam inequality should be to  restrict the investigation to a sub-class of
  probability distributions and/or some moment constraints.  }
% Dans une  telle construction,
%lorsque  loi cible tend  vers la  Gaussienne/puissance par  exemple, la  phi va
...
\section{\Avoir{titre de lappendix}}
