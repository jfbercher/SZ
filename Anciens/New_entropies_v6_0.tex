...
\begin{proof}
  ...
  \dmu(x) \right|\\[2mm]
  %
%  & \le & \int_\X \left| x^t \left( \frac{\nabla_x f(x)}{g(x)} \right) \right|
%  g(x) \dmu(x)\\[2mm]
%  %
%  & \le & \int_\X \| x \|_\chi \, \left\| \frac{\nabla_x f(x)}{g(x)}
%  \right\|_{\chi*} g(x) \dmu(x)\\[2mm]
  %
  ...
  \]
%since
%The first inequality is  an equality if $x f'(x) \ge 0$,  that is when $|x|$ and
%$f(x)$  have  the same  sense  of  variation.  Thus,  for  such  a density  $f$,
%satisfying
%
\[
...
\bibliography{\folder/BiblioIT,\folder/BiblioPhys,\folder/BiblioTS,\folder/BiblioMath}
\end{document}
