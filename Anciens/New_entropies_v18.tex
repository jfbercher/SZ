...
\abstract{\SZ{In this paper we focus on extended informational measures based on
    a   convex  function   $\phi$  --entropies   extended  Fisher   information,
    generalized moments--.  Both the generalization of  the Fisher's information
    and  the moments  are based  on the  definition of  an escort  distribution,
    precisely based on the (entropic) function $\phi$. We thus revisit the usual
    maximum entropy  principle --more  especially its inverse  problem, starting
    from the distribution and constraints--,  which conduct to a wider extension
    of the $\phi$-entropy  with a state-dependent entropic  functional. Then, we
    generalize some  interrelations between  the extended  informationa measures
    --Cramer-Rao inequality, de Bruijn's identity--  in this broader context. In
    this particular framework, the maximum entropy distributions play en central
    role. All the results derived in the paper include the usual ones as special
    cases.}
%%   In this  paper we  study extend  well-know informational  measure --Shannon
%%   entropy,  Fisher   information,  statistical  moments--  and   revisit  the
%%   relationships they satisfy.   % iedbetween usual information  measures in a
%%   wider  context.   More  precisely,  we consider  generalizations  of  these
%%   measure,  generated   by  a   convex  function   $\phi$:  $\phi$-entropies,
%%   $\phi$-fisher  information  and  generalized moments  via  a  $\phi$-escort
%%   distribution.  First,  we  study  the maximum  entropy  problem,  and  more
%%   specially the  inverse problem  --search for  the entropy  so that  a given
%%   distribution  is  its  maximizer--.  We   then  esis  paper  focus  on  the
%%   interrelations between informational measures defined via a convex function
%%   $\phi$.  maximum  entropy p%roblems  under moment constraints.  Contrary to
%%   the  usual problem  of finding  the  maximizer of  a given  entropy, or  of
%%   selecting constraints such that a given  distribution is a maximizer of the
%%   considered entropy, we consider here the problem of the determination of an
%%   entropy such  that a given  distribution is its  maximizer. Our goal  is to
%%   adapt  the  entropy  to  its   maximizer,  with  potential  application  in
%%   entropy-based goodness-of-fit tests for instance.  It allows us to consider
%%   distributions outside the  exponential family – to which  the maximizers of
%%   the  Shannon  entropy  belong  -,   and  also  to  consider  simple  moment
%%   constraints, estimated from  the observed sample. Out  approach also yields
%%   entropic  functionals that  are function  of both  probability density  and
%%   state, allowing us to include skew-symmetric or multimodal distributions in
%%   the setting.   Finally, extended  informational quantities  are introduced,
%%   such that  generalized moments  and generalized Fisher  informations.  With
%%   thes extended quantities,  we propose extended version  of the Cram\'er-Rao
%%   inequality  and of  the  de Bruijn  identity, valid  or  saturated for  the
%%   maximal  entropy  distribution  corresponding to  the  generalized  entropy
%%   previsouly studied.
}
...
\begin{Example}\label{arcsineUni:ex}
  ...
  %, \quad  \pm \frac{\pi}{2} \alpha + \gamma_\pm$,
...
\begin{Example}\label{arcsineMulti:ex}
  ...
  = 0$
  %, \: \frac{\pi}{2} \alpha_\pm + \gamma_\pm  = 0$
...
%whereas $\beta$ and $\gamma$ induce a shift).
...
%\]
%
%(again, for  non unity  $\alpha$, the  family of entropies  comes from  a scaled
%version of this entropy).
...
%Then, by the judicious  choice $\alpha = 1, \beta = 0
%= \gamma = 0$ we obtain
%%
%\[
...
non-monotonous,  unimodal, where  the  mode is  located at  \SZ{$x  = \frac{r  ,
  ...
  %, with
  %\[
  %r = \frac{a-1}{b} \qquad \mbox{and} \qquad s = \frac{\Gamma(\alpha)}{b \, (a-1)^{a-1}}
  %\]
}
%$ with
%%
...
is searched  \SZ{under the form  $u \, \sum_{l \ge  0} a_l \left[-  \W_k\left( -
  ...
  coefficients $a_l$.}
  %and   a  \SZ{power} series  of  $\left[  -
  %\W_k\left( - u^{\frac{1}{a-1}} \right) \right]$
  %and then to recognize the
